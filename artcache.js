  ...
  for (i in searchAlbums) {
    ...
    if (searchAlbums[i] !== undefined && searchAlbums[i] !== '') {
      ...
      if (filename !== null) {
          ...
          try {
            response = await axios.get(`https://musicbrainz.org/ws/2/release-group?query=%22${encodeURIComponent(searchAlbums[i].toLowerCase())}%22%20AND%20artist:%22${encodeURIComponent(searchArtist.toLowerCase())}%22`, mbOptions)
            if (response.data['release-groups'].length > 0) {
              const rg = response.data['release-groups'][0]
              ...
              try {
                  ...
                  for (j in artTypes) {
                    if (artTypes[j] in fanartData.data) {
                      ...
                      if (artistFilename != null) {
                        ...
                        response = await axios({ url: fanartData.data[artTypes[j]][0].url, method: 'GET', responseType: 'stream' })
                ...
                if (fanartKeys !== null && fanartKeys.includes(rg.id)) {
                  ...
                  if ('albumcover' in art) {
                    ...
                    response = await axios({ url: art.albumcover[0].url, method: 'GET', responseType: 'stream' })
